...
stream_key_init(int method, const char *pass, const char *key)
{
    ...
    if (method == SALSA20 || method == CHACHA20 || method == CHACHA20IETF) {
        ...
        cipher->info->iv_size    = supported_stream_ciphers_nonce_size[method];
				...
				cipher->info->MBEDTLS_PRIVATE(key_bitlen) = supported_stream_ciphers_key_size[method] * 8;
				cipher->info->MBEDTLS_PRIVATE(iv_size) = supported_stream_ciphers_nonce_size[method];
#endif
    } else {
